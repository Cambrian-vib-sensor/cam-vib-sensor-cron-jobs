...
async function saveEmail(email) {
    if (!(email.value instanceof Error)) { 
        try {
            const headers = email.value.data.payload.headers;
            const gmailid = email.value.data.id;
...
async function testEmail() {
    ...
    console.log(emails);
    if (emails.length) {
        await query('INSERT INTO sensordata1 (sensor_id, sensor_value, received_at, gmail_id) VALUES ?', [emails]);
    }
    pool.end();
}
...
var startDt = "2022-10-03T23:59:59"; 
var endDt = "2022-10-05T00:00:00";
...
run().then(()=>{pool.end();console.log("Completed")});
