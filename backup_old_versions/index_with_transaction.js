...
async function alertCheck() {    
    ...
    if (minutesnow == 0 || minutesnow == 30) {
        ...
        const [result, f] = await connection.execute('Select `sensor_id` from (SELECT distinct `sensor_id`, max(`received_at`) as `maxi` FROM `sensordata` group by `sensor_id` order by substring(`sensor_id`, 1, 4), `maxi` DESC) A group by substring(`sensor_id`, 1, 4)').catch(err=>{throw err});
        if (result.length) {
