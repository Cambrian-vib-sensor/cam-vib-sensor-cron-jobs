...
async function alertCheck() {    
    ...
    if ((minutesnow >= 0 && minutesnow < 5) || (minutesnow >= 30 && minutesnow < 35)) {
...
async function limitCheck() {
    console.log("Checking sensor data limits");
    startDt.setSeconds(startDt.getSeconds() + 1); //Check from last checked time.
    ...
    await Promise.all(sensors.map(async item => {
        ...
        let [sensordata, f3] = await connection.execute('SELECT * from sensordata where sensor_id like ? and sensor_value > ? and received_at > ?', [sensor_id, item.vibration_max_limit, formatDateTime(startDt)]).catch(err => {throw err});

        await Promise.all(sensordata.map(async item => {
            //console.log(item);
            let subject = item.sensor_id + ' sensor value exceeded to ' + item.sensor_value + ' at ' + item.received_at.toLocaleString();
...
connect().then(authorize).then(listEmails).then(limitCheck).then(alertCheck).then(() => console.log("Success")).catch(error=>console.log(error.message)).finally(()=>{connection.end();/*pool.end();*/ console.log("Completed")}); //Run once
