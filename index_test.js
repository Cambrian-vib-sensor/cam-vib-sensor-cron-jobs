...
async function getEmail(message) {
    ...
    /*const [result, f] = await connection.execute('SELECT count(*) as cnt from sensordata where gmail_id = ?', [message.id]);
    if (result[0].cnt) {
...
async function connect() {
    try{
        connection = await mysql.createConnection({ host: DB_HOST, 
            port: DB_PORT, 
            database: DB_NAME, 
            user: DB_USER,
            password: DB_PWD,
    ...
    if (!(email instanceof Error)) { //In case the message already exists, getEmail returns message already exists error, if the checking is included
        try {
            if (email.data.snippet.includes("Velocity -&gt;")) { //Gmail filtering for the search term Velocity does not work. Hence filtered here.
                ...
                const [result, f] = await connection.execute('SELECT count(*) as cnt from sensordata where sensor_id = ? and received_at = ?', [sensorid, receiveddate]);
                if (!result[0].cnt) {
                    const emailObj = [sensorid, sensorvalue, receiveddate, gmailid];
                    this.push(emailObj);
                    //Batch insert is better than individual insert
                    //let result = await connection.query('INSERT INTO sensordata (sensor_id, sensor_value, received_at, gmail_id) VALUES (?,?,?,?)', [sensorid, sensorvalue, receiveddate, gmailid]);
                }
            }
        } catch (err) {
            throw err;
...
async function listEmails(auth) {
    ...
    const [result, f] = await connection.execute('SELECT max(received_at) as startDt from sensordata').catch(err => {throw err});    
            ...
            if (res.data && res.data.messages) { //res.data.resultSizeEstimate > 0; resultSizeEstimate gives the size per page
                ...
                await Promise.all(res.data.messages.map(getEmail, getEmailOptions)).then(async (messages) => { 
                    ...
                    if (emails.length) {
                        ...
                        await connection.query('INSERT INTO sensordata (sensor_id, sensor_value, received_at, gmail_id) VALUES ?', [emails]);
                    } else {
...
findUnresponsiveSensors = async row => {           
    const [lastReceived, f] = await connection.execute('SELECT max(received_at) as startDt from sensordata where sensor_id = ?', [row.sensor_id]).catch(err => {throw err}); 
...
async function sendEmail(subject, emailbody){
...
async function alertCheck() {    
    ...
    //if (minutesnow == 0 || minutesnow == 30) {
        ...
        //const [result, f] = await connection.execute('SELECT distinct sensor_id from sensordata');
        //Select sensors linked to current locations only
        const [result, f] = await connection.query('Select `sensor_id` from (SELECT distinct `sensor_id`, max(`received_at`) as `maxi` FROM `sensordata` group by `sensor_id` order by substring(`sensor_id`, 1, 4), `maxi` DESC) A group by substring(`sensor_id`, 1, 4)').catch(err=>{throw err});
        if (result.length) {
            await Promise.all(result.map(findUnresponsiveSensors)).then(async (messages) => {
                ...
                if (emaildata.length) {
                    ...
                    let subject = 'Unresponsive Sensors Alert';
                    console.log(emailbody);
                    //await sendEmail(subject, emailbody).catch(err => {throw err});
                }
            }).catch(err => {throw err});
        }
    //}
...
async function limitCheck() {
    let [result, f1] = await connection.execute('SELECT max(received_at) as startDt from sensordata').catch(err => {throw err});    
    if (result[0].startDt) { //Check later what it returns if table is empty
        startDt = new Date(result[0].startDt)
    }
    ...
    let [sensors, f2] = await connection.execute('SELECT * from sensors where vibration_max_limit is not null').catch(err => {throw err});

    await Promise.all(sensors.map(async item => {
        console.log(item.serial_no.substring(0,4) + "%" + item.vm + "%" );
        let sensor_id = item.serial_no.substring(0,4) + "%" + item.vm + "%";
        let [sensordata, f3] = await connection.execute('SELECT * from sensordata where sensor_id like ? and sensor_value > ? and received_at > ?', [sensor_id, item.vibration_max_limit, "2022-11-10 12:00:00"]).catch(err => {throw err});

        await Promise.all(sensordata.map(async item => {
            console.log(item);
            let subject = 'TEST EMAIL: ' + item.sensor_id + ' sensor value exceeded to ' + item.sensor_value + ' at ' + item.received_at.toLocaleString();
            let emailbody = subject;
            await sendEmail(subject, emailbody).catch(err => {throw err});
        })).catch(err=>{throw err});
    })).catch(err=>{throw err});

    //let [sensordata, f3] = await connection.execute('SELECT * from sensordata where received_at > ?', [formatDateTime(startDt)]).catch(err => {throw err});
...
//connect().then(authorize).then(listEmails).then(alertCheck).then(() => console.log("Success")).catch(error=>console.log(error.message)).finally(()=>{connection.end();/*pool.end();*/ console.log("Completed")}); //Run once
